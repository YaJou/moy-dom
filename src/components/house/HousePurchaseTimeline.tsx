...
export function HousePurchaseTimeline() {
  return (
    <section className="mt-10">
      ...
      <div className="mt-6 flex flex-col gap-0 md:flex-row md:items-start md:justify-between">
        {purchaseSteps.map((step, i) => (
          <div key={step.step} className="flex flex-1 flex-col items-center text-center">
            ...
            {i < purchaseSteps.length - 1 && (
              <div className="my-3 hidden h-0.5 flex-1 bg-primary/30 md:block md:w-full md:translate-y-5" aria-hidden />
            )}
            {i < purchaseSteps.length - 1 && (
              <div className="my-2 text-primary md:hidden" aria-hidden>↓</div>
            )}
          </div>
        ))}
      </div>
